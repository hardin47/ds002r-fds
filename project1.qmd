...
For each of the two analyses, find a TidyTuesday dataset and create a plot (start with a qmd file and create an html file).  Your YAML will look something like what I have below (note that the format here is html, unlike the pdf that we use for the homework). Some considerations:

* Each TidyTuesday analysis has a separate .qmd file.
* The two analyses compile to two different .html files and are linked via a drop-down menu (see example `_quarto.yml` file below).
...
Mini-Project 1 must be submitted on Canvas (not Gradescope) by **11:59 PM on Wednesday September 30, 2026**.  All you need to submit is a URL for your website (or private GitHub repo if you take that route)!
