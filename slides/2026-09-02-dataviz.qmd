...
**Color** (either) to what extent? how severely? Beware of red/green color blindness. Use color-vision-safe palettes like <a href = "https://ggplot2.tidyverse.org/reference/scale_viridis.html" target = "_blank">`viridis`</a> or <a href = "https://malcolmbarrett.github.io/ggokabeito/" target = "_blank">`Okabe–Ito`</a>.
...
ggplot(mtcars, aes(x = mpg, y = reorder(rownames(mtcars), mpg))) +
  geom_point(size = 3) +
  labs(title = "Which visual cue is most prominent?", x = "MPG", y = "") +
  theme_minimal()
```

Visual cues^[Probably the most prominent cue is position.]
...
Visual cues^[Probably the most prominent cue is length.]
...
mtcars |>
  ...
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Which visual cue is most prominent?", fill = "Cylinders") +
  labs(x = "", y = "") +
  theme_minimal()
```

Visual cues^[Angle, color, and area all seem to be enconding the same information.]
...
ggplot(economics, aes(x = date, y = unemploy)) +
  geom_line() +
  labs(
    title = "Which visual cue is most prominent?",
    x = "Date",
...
Visual cues^[Direction. Maybe length.]
...
ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, shape = Species)) +
  ...
  labs(title = "Which visual cue is most prominent?") +
  theme_minimal()
```


Visual cues^[Shape is super important here, but we also see position.]
...
mtcars |>
  group_by(cyl, gear) |>
  summarize(ave_mpg = mean(mpg, na.rm = TRUE)) |>
  ggplot(aes(x = factor(cyl), y = factor(gear), fill = ave_mpg)) +
  geom_tile() +
...
1.  One of the best ways to get started with ggplot is to Google what you want to do with the word `ggplot`.  Then look through the **images** that come up. 
...
for the geoms, but each geom may change the defaults.
...
* Make it easy to distinguish elements (e.g., color)
...
- Make your website accessible, <a href = "https://www.apreshill.com/blog/2022-04-we-dont-talk-about-quarto/#what-youll-like" target = "_blank">how to add alt text</a>. Feel free to use (and then check!!!) the <a href = "https://numbats.github.io/autoAlt/index.html" target = _blank">`autoAlt` package</a> in R which automatically generates alt text.
...
```{r fig.cap = "How many aspects of this graph can you point out which are relevant to figuring out that cholera infection was coming from a single pump?  Are there any distracting aspects?", out.width="800px", echo=FALSE}
...
::: {.column width="50%"}
```{r, eval=TRUE}
ggplot(data = Births2015, aes(x = date, y = births, fill = wday)) +
  geom_area() +
  labs(title = "US Births in 2015")
```
...
HELP_data <- left_join(HELPrct, HELPfull, by = c("id" = "ID")) |>
  filter(!is.na(A11E)) |>
  mutate(children = if_else(A11E == 0, "no", "yes"))
```
...
#| eval: false
#| code-line-numbers: "3"
ggplot(data = HELP_data, aes(x = substance)) +
  geom_bar() +
  labs(title = "HELP trial")
```
...
HELP_data |>
  ggplot(aes(x = substance, fill = children)) +
  geom_bar() +
  labs(title = "HELP trial")
```
:::
...
#| eval: false
#| code-line-numbers: "3"
ggplot(data = HELP_data, aes(x = substance, fill = children)) +
  geom_bar() +
  labs(title = "HELP trial")
```
...
::: {.column width="50%"}
```{r message=FALSE, warning=FALSE, echo=FALSE}
ggplot(HELP_data, aes(x = substance, fill = children)) +
  geom_bar(position = "fill") +
  labs(title = "HELP trial", y = "actually, percent")
```
:::
...
#| eval: false
#| code-line-numbers: "3"
ggplot(data = HELP_data, aes(x = age)) +
  geom_histogram() +
  labs(title = "HELP trial")
```
...
#| eval: true
#| code-line-numbers: "3,4"
ggplot(data = HELP_data, aes(x = i1, color = substance, linetype = children)) +
  geom_line(stat = "density") +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
#| eval: true
#| code-line-numbers: "4"
ggplot(data = HELP_data, aes(x = i1, color = substance)) +
  geom_line(stat = "density") +
  facet_grid(. ~ children) +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
ggplot(data = HELP_data, aes(x = substance, y = age, color = children)) +
  ...
  geom_point(
    aes(color = children, fill = children),
