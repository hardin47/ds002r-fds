...
21 tables in the `imdb` database!  Lots of details on movies, TV series, video games, and more. Today interest is in TV series (and details within, like actors and ratings).
...
Still using a `{sql}` chunk.  The `DESCRIBE` command shows the 18 field names (variables) in the `aka_title` table.  Some of the variables are characters (`text`) and some are numeric (either `double` or `bigint`)
...
DO NOT RUN:  SELECT * FROM aka_name;
```
...
SELECT * FROM keyword LIMIT 0, 14;
...
How do we know how many titles are in the database?  Two different ways of counting the rows (that produce the same number):

```{sql}
#| connection: con_imdb

SELECT COUNT(*), SUM(1) FROM keyword LIMIT 0, 14;
```

```{sql}
#| connection: con_air

SELECT COUNT(*), SUM(1) FROM flights LIMIT 0, 14;
```

More than 500,000 titles and 1.3 million actors.
