...
- See the <a href = "https://rstudio.github.io/cheatsheets/html/purrr.html" target = "_blank">purrr cheatsheet</a>
...
- use `\` as a shortcut for the word `function`, to set a formula (when the function is neither a single name nor defined by `function(...) { ...} `)
...
```{r, eval=TRUE}
map_dbl(.x = fastfood, .f = \(happy) mean(happy, na.rm = TRUE))
```
...
map_dbl(fastfood, \(x) length(unique(x)))
map_dbl(fastfood, \(chirp) length(unique(chirp)))
...
#| error: true
#error
map_dbl(fastfood, \(x) length)
#error
map_dbl(fastfood, \(x) length(unique()))
#error
map_dbl(fastfood, \(y) length(unique(x)))
```
...
- `map2_*()` has two arguments, `.x` and `.y` (the arguments must be `.x` and `.y`, but the the function call can be on any variables)
...
- `imap()` is like `map2()`except that `.y` (i.e., the second argument) is derived from `names(.x)` if the data frame has names.
...
```{r}
imap_chr(.x = fastfood, .f = \(x, y) {
  paste(y, "has a mean of", round(mean(x), 1))
}) |>
  head()

map2_chr(.x = fastfood, .y = names(fastfood), .f = \(x, y) {
  paste(y, "has a mean of", round(mean(x), 1))
}) |>
  head()
```
...
```{r}
expand_grid(n = c(1, 2, 3), min = c(1, 10), max = c(10, 100))
