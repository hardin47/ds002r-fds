...
Use the <a href = "https://rstudio.github.io/cheatsheets/html/strings.html" target = "_blank">**stringr** cheat sheet</a>. 
...
new_fruit <- data.frame(words = x) |>
  mutate(
    x_recode = fct_recode(words, fruit = "apple", fruit = "banana"),
    x_rewrite = if_else(
      words == "apple",
      "fruit",
      if_else(words == "banana", "fruit", words)
    )
  )
...
The **forcats** package within tidyverse contains lots of functions to help process factor variables Use the <a href = "https://rstudio.github.io/cheatsheets/html/factors.html" target = "_blank">**forcats** cheat sheet</a>. We'll focus on the most common functions. 
...
flightsWK |>
  select(year, month, day, ymdday, weekdy, whichweek, dep_time, arr_time)
```
