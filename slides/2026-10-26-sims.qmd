...
data.frame(value = c(-2:2)) |>
  mutate(abs_value = if_else(value >= 0, value, -value)) # abs val
```
...
sample(alph, size = 5, replace = FALSE) # sample (e.g., from a population)

sample(alph, size = 5, replace = TRUE) # sample (e.g., from a population)

sample(alph, size = 10, replace = FALSE) # shuffle

sample(alph, size = 10, replace = TRUE) # resample
```
...
meet_func <- function(nada) {
  data.frame(
    ...
    mutate(
      result = if_else(abs(alex - jordan) <= 10, "They meet", "They do not")
    )
}
```
...
meet_vect <- data.frame(
  ...
  mutate(result = if_else(abs(alex - jordan) <= 10, "They meet", "They do not"))
toc()
```
...
```{r}
map_lgl(c(1:10), \(z) hat_match(n = 10))
...
map_lgl(c(1:num_iter), \(z) hat_match(n = 10)) |>
  mean()
```
...
hat_match_prob <- function(n, reps) {
  prob <- map_lgl(c(1:reps), \(z) hat_match(n = n)) |>
    mean()
...
class_duplicates <- function(unicorn) {
  ...
  return(if_else(num_duplicates > 0, 1, 0))
}
...
set.seed(47)
num_stud <- 28
map_dbl(c(1:10), \(w) class_duplicates(class_size = num_stud))
```
...
num_stud <- 28
num_class <- 1000
map_dbl(c(1:num_class), \(w) class_duplicates(class_size = num_stud)) |>
  mean()
```
...
birth_match_prob <- function(num_stud, reps) {
  prob <- map_dbl(c(1:reps), \(w) class_duplicates(class_size = num_stud)) |>
    mean()
...
new.college.data <- new.college.data |>
  mutate(
    fp = if_else(talent < 115 & predicted > 115, 1, 0),
    tp = if_else(talent > 115 & predicted > 115, 1, 0),
    fn = if_else(talent > 115 & predicted < 115, 1, 0),
    tn = if_else(talent < 115 & predicted < 115, 1, 0)
  )
...
new.college.data.glb <- new.college.data |>
  mutate(
    fp = if_else(talent < 115 & global.predicted > 115, 1, 0),
    tp = if_else(talent > 115 & global.predicted > 115, 1, 0),
    fn = if_else(talent > 115 & global.predicted < 115, 1, 0),
    tn = if_else(talent < 115 & global.predicted < 115, 1, 0)
  )
...
calculate_return <- function(step, alpha) {
  ...
  return(data.frame(
    step = step,
    return_rate = (1 - alpha) * risk_free_rate + alpha * risky_rate
  ))
}
...
pmap(
  data.frame(step = c(1:decision_steps), alpha = rep(0.5, decision_steps)),
  calculate_return
) |>
  list_rbind()
```
...
run_func <- function(run, steps, alpha) {
  pmap(
    data.frame(step = c(1:steps), alpha = rep(alpha, steps)),
    calculate_return
  ) |>
    list_rbind() |>
...
map(c(1:runs), run_func, steps = decision_steps, alpha = 0.5) |>
  list_rbind()
```
...
map(c(1:runs), run_func, steps = decision_steps, alpha = 0.5) |>
  list_rbind() |>
  group_by(run_number) |>
  mutate(compound_return = cumprod(return_rate))
```
...
#| echo: false
runs <- 20
map(c(1:runs), run_func, steps = decision_steps, alpha = 0.1) |>
  list_rbind() |>
  group_by(run_number) |>
  mutate(compound_return = cumprod(return_rate)) |>
  ggplot(aes(x = step, y = compound_return)) +
...
runs <- 20
map(c(1:runs), run_func, steps = decision_steps, alpha = 0.9) |>
  list_rbind() |>
  group_by(run_number) |>
  mutate(compound_return = cumprod(return_rate)) |>
  ggplot(aes(x = step, y = compound_return)) +
...
#| code-line-numbers: "2"
runs <- 20
map(c(1:runs), run_func_sd10, steps = decision_steps, alpha = 0.5) |>
  list_rbind() |>
  group_by(run_number) |>
  mutate(compound_return = cumprod(return_rate)) |>
  ggplot(aes(x = step, y = compound_return)) +
